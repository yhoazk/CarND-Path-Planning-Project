#include <iostream>
    ...
    vector<double> JMT(vector< double> start, vector <double> end, double T)
    {
        ...
        */
        ...
        double t3 = T*t2;
        double t4 = t3*T;
        A << t3,t4,T*t4,  3*t2, 4*t3, 5*t4,  6*T, 12*t2, 20*t3;
        B << end[0]- (start[0] + start[1]*T + 0.5f * start[2]*t2), end[1] - (start[1]+ start[2]*T), end[2]-start[2];
        ColPivHouseholderQR<Matrix3f> dec(A);
        //Eigen::Vector3f x = A.colPivHouseholderQr().solve(B);
        x << dec.solve(B);
       // x2 << start, dec.solve(B);
       cout << start[0]<<", " << start[1]<<", " << start[2]<<", " <<  x[0]<<", " << x[1]<<", " << x[2]<< endl;
